...
def build_query_plan_prompts(semantic_prompt_text: str, history_text: str, question: str) -> tuple[str, str]:
    system_prompt = (
        ...
        '3) dimensions、metrics 必须返回中文业务名称；sql 输出列别名尽量与其一致。'
        '4) 只有用户明确要求按某维度拆分时才 GROUP BY；否则返回整体汇总。'
        '5) 排名、TOP、前N 未给排序指标时先澄清。'
        '6) 销售/订单分析优先 order_master；品牌/产品/品类/SKU 分析优先 order_detail；用户属性优先 user_info；门店和区域优先 store_info；退款优先 refund_master 或 refund_detail。'
        '7) 若问销售金额、销量、GMV且未指定状态，默认统计 已支付、已发货、已完成、部分退款。'
        '8) 商品粒度销售金额必须用 order_detail.line_paid_amount 或 line_gross_amount。'
        '9) 下单日期只能用 created_at 或 DATE(order_master.created_at)，禁止使用 order_date、pay_date、ship_date 等虚拟列。'
        '10) 只能使用候选语义层里真实出现的表、字段和关联关系；sql 只能是 SELECT 或 WITH；未限制条数时默认 LIMIT 200。'
        '11) 问题带时间语义时，必须返回对应的 time_granularity 和时间范围；没有时间则返回 none。'
    )
...
def build_sql_repair_prompts(semantic_prompt_text: str, history_text: str, question: str, failed_sql: str, error_message: str) -> tuple[str, str]:
    system_prompt = (
        ...
        '规则：'
        '1) 只允许 SELECT 或 WITH。'
        '2) 优先修复 MySQL 方言问题、引号、日期写法、join、聚合和别名错误。'
